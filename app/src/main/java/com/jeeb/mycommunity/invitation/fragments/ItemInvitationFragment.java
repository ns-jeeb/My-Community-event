 ...
 */
public class ItemInvitationFragment extends Fragment implements InvitationHelper.OnLoadInvitation {
    ...
    public static ItemInvitationFragment newInstance(String token) {
        ItemInvitationFragment fragment = new ItemInvitationFragment();
...
//                Glide.with(getActivity()).load(R.drawable.ic_lock_open).into(binding.imgBrideGroom);
                binding.imgBrideGroom.setVisibility(View.GONE);
...
//                getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.main_content,new DisplayInvitationFragment()).commit();
                binding.imgBrideGroom.setVisibility(View.VISIBLE);
            }
